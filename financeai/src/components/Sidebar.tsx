...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    { id: 'tablero' as NavigationTab, label: 'Panel', icon: LayoutGrid },
    { id: 'reportes' as NavigationTab, label: 'Informes', icon: BarChart3 },
    { id: 'historial' as NavigationTab, label: 'Historial', icon: History },
    ...
    <div className="flex flex-col justify-between h-full">
      <div>
        ...
        <div className="flex items-center justify-between mb-8">
          <div 
            id="brand-header"
            onClick={() => handleTabClick('tablero')}
        ...
        <nav className="space-y-1.5" id="main-navigation-menu">
          {navItems.map((item) => {
            ...
            const isActive = currentTab === item.id || (item.id === 'perfil' && currentTab === 'configuracion');
            return (
              <button
                key={item.id}
                id={`nav-btn-${item.id}`}
