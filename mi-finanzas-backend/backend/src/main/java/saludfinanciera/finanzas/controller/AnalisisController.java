...
public class AnalisisController {
    ...
    @Operation(
            summary = "Obtener historial por ID de usuario paginado",
            description = "Consulta la lista paginada de análisis financieros históricos asociados a un ID de usuario específico."
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<Page<AnalisisFinanciero>> obtenerHistorialPorUsuario(
            @PathVariable Long usuarioId,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        Pageable pageable = PageRequest.of(page, Math.min(size, 100));
        Page<AnalisisFinanciero> historial = analisisService.obtenerHistorialPorUsuario(usuarioId, pageable);
        return ResponseEntity.ok(historial);
    }

    @Operation(
            summary = "Obtener historial general paginado",
            description = "Consulta el historial financiero paginado del primer usuario activo registrado en el sistema."
    )
    @ApiResponse(responseCode = "200", description = "Historial general obtenido exitosamente")
    @GetMapping("/historial")
    public ResponseEntity<Page<AnalisisFinanciero>> obtenerHistorialGeneral(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        Pageable pageable = PageRequest.of(page, Math.min(size, 100));
        Page<AnalisisFinanciero> historial = analisisService.obtenerHistorialGeneral(pageable);
        return ResponseEntity.ok(historial);
    }
}